...
describe('detectDuplicateIssue', () => {
  ...
  it('returns null when no matches exceed threshold', async () => {
    const docs = [makeDocSnap('i1', { title: 'Unrelated noise complaint', description: 'Loud music', location: { geohash: 'xxxxx' } })];
    mockGet.mockResolvedValueOnce({ forEach: (cb: (d: typeof docs[0]) => void) => docs.forEach(cb) });
    const result = await detectDuplicateIssue(input);
    expect(result).toBeNull();
  });
  ...
  it('returns best match when score >= 0.45', async () => {
    const docs = [
      makeDocSnap('i1', { title: 'Large pothole on Main St', description: 'A deep pothole near the intersection.', location: { geohash: 'dr5reg' } }),
    ];
    mockGet.mockResolvedValueOnce({ forEach: (cb: (d: typeof docs[0]) => void) => docs.forEach(cb) });
    const result = await detectDuplicateIssue(input);
  ...
  it('sorts results by score descending and returns highest', async () => {
    const docs = [
      makeDocSnap('low', { title: 'Pothole on Main St', description: 'Pothole near intersection.', location: { geohash: 'dr5reg' } }),
      makeDocSnap('high', { title: 'Large pothole on Main St', description: 'A deep pothole near the intersection. Very dangerous.', location: { geohash: 'dr5reg' } }),
    ];
    mockGet.mockResolvedValueOnce({ forEach: (cb: (d: typeof docs[0]) => void) => docs.forEach(cb) });
    const result = await detectDuplicateIssue(input);
    expect(result).not.toBeNull();
  ...
  it('returns null for empty title/description (both blank)', async () => {
    const docs = [makeDocSnap('i1', { title: '', description: '', location: { geohash: 'dr5regxx' } })];
    mockGet.mockResolvedValueOnce({ forEach: (cb: (d: typeof docs[0]) => void) => docs.forEach(cb) });
    const result = await detectDuplicateIssue({ ...input, title: '', description: '' });
    expect(result).toBeNull();
  });
});
