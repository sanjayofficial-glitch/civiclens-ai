...
async function backfillBadges() {
  ...
  issuesSnap.forEach(doc => {
    const data = doc.data();
    if (data.reporterId) {
      userIssueCounts.set(data.reporterId, (userIssueCounts.get(data.reporterId) || 0) + 1);
    }
  });

  // Now update users and award badges
  console.log(`Found ${userIssueCounts.size} users with issues.`);
  
  for (const [userId, count] of userIssueCounts.entries()) {
    console.log(`Processing user ${userId} with ${count} reported issues...`);
